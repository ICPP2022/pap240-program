...
!Input: iseed
!Output: poly(1:101), other, offset, index_r
...
!   INDEX_R must be initialised to an integer between 1 and 101 inclusive,
...
n = INDEX_R - 64
IF (n <= 0) n = n + 101
x = poly(INDEX_R) + poly(INDEX_R)
x = xmod4 - poly(n) - poly(n) - x - x - poly(INDEX_R)
IF (x < zero) THEN
  IF (x < -xmod) x = x + xmod2
  IF (x < zero) x = x + xmod
ELSE
...
poly(INDEX_R) = x
INDEX_R = INDEX_R + 1
IF (INDEX_R > 101) INDEX_R = INDEX_R - 101
